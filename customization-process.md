...
| 표 헤더 배경 / 글씨 | 회색 `#E8E8E8` + 검정 글씨 | "표 헤더 배경색·글자색 — 회색 유지? 네이비? 다른 색? 글씨 색도 같이 지정." |
| 표 헤더 바로 아래 선 | 검정 1pt | "헤더 아래 구분선 굵기·컬러는? (예: 네이비 1.5pt)" |
| 표 외곽 상/하 테두리 | 검정 1pt | "표 맨 위·맨 아래 선 굵기·컬러는? (예: 네이비 1.5pt)" |
| 표 외곽 좌/우 테두리 | **없음** (선 표시 안 함) | 사용자 언급 없으면 기본값(없음) 유지. 명시적으로 넣어달라 하면 "굵기·컬러는?" 확인. |
| 표 내부 선(행 구분·열 구분) | 회색 0.5pt | "내부 선(상/하/좌/우) 굵기·컬러는? 다 같이? 행과 열 따로?" |

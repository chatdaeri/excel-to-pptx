...
│
└── examples/                                   ← 사용자 엑셀 템플릿
    ├── sample-template.xlsx                      설명이 셀 안에 들어있는 1시트 템플릿
    └── build-template.py                         템플릿 재생성 스크립트 (openpyxl)
```
...
사용자에게 가장 먼저 안내할 것: **`examples/sample-template.xlsx` 열어서 셀 안 설명을 자기 데이터로 교체**. 파일 구조·네이밍 규약·블록 구분이 모두 셀 안 텍스트로 들어있음.

요약:

```
시트명      = 01_대목차_소목차           ← 슬라이드 1장 = 시트 1개
A1          = 메인 메시지                ← 한 줄 결론 (18pt Medium 렌더)
A2          = 서브 메시지                ← 범위·조건 (11pt Medium)
3행         = 빈 행 (블록 구분)
A4          = [표] 표 제목               ← prefix 필수
5행         = 헤더
6행~        = 데이터 (10~15행 권장)
...
### 시트 이름 / 메시지 fallback 규약

파서·런너가 자동 처리하는 케이스 (사용자가 규약 안 지켜도 에러 안 남):

| 상황 | 결과 |
|---|---|
| 시트명 `01_대목차_소목차` | `Ⅰ. 대목차 / 소목차` (정상) |
| 시트명 `대목차_소목차` (번호 없음) | `대목차 / 소목차` (소목차 박스 생성) |
| 시트명 `01_소목차` 또는 `거래량` (토큰 1개) | **대목차만 뽑힘 → 런너가 첫 블록 제목을 소목차로 보강** |
| 시트명 완전히 비거나 무의미 | **직전 divider `chapter` 를 대목차, 첫 블록 제목(또는 시트명)을 소목차로 보강** |
| A1(메인) 비어있고 A2(서브)만 있음 | 메인 빈 박스로 렌더, 서브만 표시 |
| A1·A2 **둘 다 비어있음** | **Claude 가 블록 내용을 보고 deck.cjs 에 `mainMessage`/`subhead` 를 직접 기입해야 함** (아래 섹션 참조) |

### 🤖 Auto-message 케이스 — 메인·서브 둘 다 빈 경우

`scan-sheets.py` 가 해당 시트에 `⚠ 메인·서브 모두 비어있음` 표시를 내면:

1. Claude 가 그 시트의 **모든 블록(제목·헤더·데이터·출처)** 을 읽음
2. **메인 메시지 1줄** 작성 — 그 데이터가 말하는 **결론·시사점** (예: "서울 분양가 상승세 3년 연속 두 자릿수")
3. **서브 메시지 1줄** 작성 — **범위·조건·단위** (예: "서울 25개구 / 2022~2024 / 월별 평균")
4. deck.cjs 의 해당 슬라이드에 `mainMessage` / `subhead` 로 직접 기입

엑셀 원본은 수정하지 않음. 런너는 A1·A2 가 비어도 에러 안 내고 경고 로그만 출력 — 그러나 **PPT 메시지 영역이 비어서 나감**. 반드시 Claude 가 deck.cjs 에 채워 넣을 것.
